...
dental-crm/src/lib/conversions/google-ads-client.ts                      (new error classes + listAccessibleCustomers + listConversionActions + loadGoogleAdsOAuthOnly + helpers; GoogleAdsConfig: customer_id and conversion_action_resource_name now optional; refined 401 classification per §3 row L — UNAUTHENTICATED only, not every 401)
dental-crm/src/lib/conversions/__tests__/google-ads-client.test.ts       (15 → 25 tests; +2 for the refined 401 classification)
