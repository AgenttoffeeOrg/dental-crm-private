...
| J | `createServerSupabaseClient()` in `src/lib/supabase-server.ts` correctly reads the `@supabase/ssr` session cookie. | It used the **deprecated single-cookie `get(name)`/`set(name,…)`/`remove(name)` API**. Modern `@supabase/ssr` (`^0.5.1`) chunks the session JWT across multiple cookies (e.g. `sb-<ref>-auth-token.0`, `…1`); the legacy adapter looks up the unchunked name, finds nothing, and `auth.getUser()` returns null even when the user is fully authenticated. Surfaced after fix I shipped: the new Google Ads page consistently rendered the "session needs to refresh" shell on every navigation, even after sign out + sign in. ~60 API route handlers using this same helper were masked from the failure because `getSupabaseAuthContext` (the API-route auth path) falls back to the `Authorization: Bearer` header sent by the browser-side client. Server Components have no such header. | Rewrote the adapter to the modern `getAll()` / `setAll(cookiesToSet)` API — the same shape `createMiddlewareClient` in `src/lib/supabase.ts` already uses. `getAll()` returns the full chunk array; `setAll()` writes them back through the existing try/catch (Server Components can't mutate cookies, route handlers can — Supabase doesn't care which side responds). Behaviour for the ~60 API route callers is unchanged in practice (they were already authenticating via Bearer token); for Server Components it's the difference between "always null" and "always works." Touched only `supabase-server.ts`; no caller-site changes required. |
...
dental-crm/src/lib/conversions/google-ads-client.ts                      (new error classes + listAccessibleCustomers + listConversionActions + loadGoogleAdsOAuthOnly + helpers; GoogleAdsConfig: customer_id and conversion_action_resource_name now optional)
dental-crm/src/lib/conversions/__tests__/google-ads-client.test.ts       (15 → 23 tests)
