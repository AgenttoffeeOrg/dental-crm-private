...
## Phase 2b.70 — Web Push (VAPID) gotchas

### VAPID keys are write-once; never rotate after launch

`NEXT_PUBLIC_VAPID_PUBLIC_KEY` + `VAPID_PRIVATE_KEY` + `VAPID_SUBJECT`
are generated by `scripts/generate-vapid.mjs`. **Once a browser has
subscribed under a key, rotating the keypair makes every existing
subscription silently un-deliverable** — the browser still POSTs to
its endpoint, but the push service rejects the signed payload.
There's no graceful migration path; users have to re-grant in
/settings → Notifications. Treat as schema-immutable.

The cron emits `skipped:true` when these env vars are unset, so a
fresh project boot doesn't spam errors before the keys are added.

### web-push must be lazy-required

Top-level `import webpush from 'web-push'` will crash Vercel cold
starts on routes that don't intend to send push (same class of bug
as the DOMPurify pattern). Stick to the lazy-require pattern in
`src/lib/notifications/push-server.ts:50`:
...
### Dead-subscription auto-prune

HTTP 404 and 410 from a push provider mean the subscription is
permanently dead (browser uninstalled, OS reset, user revoked).
`sendPushTo` already prunes the row from `push_subscriptions` on
those status codes — DO NOT add additional retry logic for these.
Other status codes (5xx, network) should surface as transient
failures, not prune.

### `push_subscriptions` writes go through service-role

RLS on `push_subscriptions` blocks INSERT/UPDATE/DELETE for everyone
except `service_role` (per migration 2b.59). The
`/api/push-subscriptions` route uses `createServiceClient()` after
authenticating the user via the user-scoped client. Don't try to let
users write directly.

### Push payload size limit

Push providers cap payloads at ~4 KB. Our payload is
`{title, body, url, taskId, tag, requireInteraction, sentAt}` —
typically <500 bytes. If you ever need to ship a bigger payload
(e.g., notification preview text), check the provider's limit (Apple
Web Push is stricter than Firefox / Chromium).

---

## Phase 2b.70 — Notifications cron gotchas

### Three crons, three responsibilities

| Path                                       | Schedule         | Sends                          |
| ------------------------------------------ | ---------------- | ------------------------------ |
| `/api/cron/task-reminders`                 | `*/5 * * * *`    | In-app reminder rows (DB)      |
| `/api/cron/task-push-notifications`        | `*/5 * * * *`    | OS-level push (browser)        |
| `/api/cron/task-morning-digest`            | `0 * * * *`      | Email digest at 8am local      |

Do NOT collapse these into one cron — the morning digest is hourly
because it scans every tenant's local time to find the 8am window.
The other two are 5-minute granularity for due-soon precision.

### Dedup columns matter

- `tasks.notified_at` — set when due-soon push fires. NEVER reset
  without intention; resetting causes re-firing.
- `tasks.notified_overdue_at` — set when overdue push fires.
- `app_users.task_morning_digest_last_sent_date` — set when digest
  sends. Reset on date rollover automatically (compared to
  `tenantLocalDateIso`).

Reassigning a task does NOT reset these — same principle as
locked-product principle 13 (historical facts stay historical). If
the new assignee wants a fresh ping, they re-ping manually.

### `EVERYONE_FANOUT_CAP = 25`

`assigned_to_everyone` push fanout is capped at 25 recipients. Beyond
that, the morning digest is the catch-all. If you ever onboard a
tenant with >25 active users who really need push at moment-of-task,
either lift the cap or build a more sophisticated bucketing strategy.

### Tenant-local timezone math

Per the 2b.71 sweep fix: `new Date("${ymd}T00:00:00")` parses in the
runtime TZ (UTC on Vercel), NOT the tenant's TZ. Use
`tenantOffsetSuffix(timezone)` from `src/lib/tasks/morning-digest.ts`
to construct an offset-aware ISO. The same pattern applies wherever
you need to resolve "today" in a tenant's timezone.

---

## Phase 2b.71 — Audit-first pattern for creates

CLAUDE.md says: "write audit row FIRST → mutate entity →
compensate-delete on mutation failure." For UPDATEs the entity already
has an id, so this is straightforward. For CREATEs, the trick is:

1. Pre-generate the entity id with `randomUUID()` from `node:crypto`.
2. Write the audit row with `entityId: <pregenerated>`.
3. INSERT with `{ id: <pregenerated>, ... }`.
4. On INSERT failure, call `deleteAuditRowServer(auditId, tenantId)`.

Examples in the codebase:
- `src/lib/tasks/log-call-outcome.ts:149` (follow-up task)
- `src/app/api/activities/[id]/accept-commitment-suggestion/route.ts:194`
- `src/app/api/deals/[id]/close-open-tasks/route.ts:142` (bulk variant)

Do NOT use the older pattern (insert → get id → audit) — if the audit
write fails, you're stuck with an unaudited entity and CLAUDE.md
requires the request to fail in that case.
