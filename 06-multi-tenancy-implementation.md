...
const switchOrg = useCallback(
  async (tenantId: string): Promise<OrgSwitchResult> => {
...
const { data: contacts } = await supabase.from('contacts').select('*').eq('tenant_id', tenantId); // ✅ Tenant filter
```
...
const shouldShowLocationSwitcher = tenant?.is_multi_location && locations && locations.length > 1;
```
