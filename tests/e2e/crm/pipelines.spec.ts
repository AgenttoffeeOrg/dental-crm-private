...
test.describe('Pipelines Workflow', () => {
  ...
  test('B2.2: Add stages (ordered, unique position)', async ({ page }) => {
    ...
    for (const stage of testPipeline.stages) {
      ...
      await expect(page.locator(`[data-stage-position="${stage.position}"]`)).toContainText(
        stage.name
      );
    }
  ...
  test('B2.4: Move deal across stages', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="activity-log"]')).toContainText(
      testPipeline.stages[1].name
    );
  });
  ...
  test('B2.5: Verify stage automations fire (if set)', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="task-linked-deal"]')).toContainText(
      'Deal to Trigger Automation'
    );
  });
  ...
  test('B2.7: Pipeline validation - stage ordering', async ({ page }) => {
    ...
    const newStage1Pos = await page
      .locator('[data-testid="stage-0"]')
      .getAttribute('data-stage-position');
    const newStage2Pos = await page
      .locator('[data-testid="stage-1"]')
      .getAttribute('data-stage-position');
