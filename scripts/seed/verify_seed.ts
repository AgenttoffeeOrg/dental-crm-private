...
async function seedUsers() {
  ...
  for (const tenant of TENANTS) {
    ...
    for (const userType of USERS_PER_TENANT) {
      ...
      const { error } = await supabase.from('app_users').upsert(user, { onConflict: 'id' });
      ...
      for (let i = 0; i < pipeline.stages.length; i++) {
        ...
        const probability =
          i === pipeline.stages.length - 1
            ? 100
            : Math.round((i + 1) * (100 / pipeline.stages.length));
...
async function seedEntitlements() {
  ...
  for (const feature of features) {
    const { error } = await supabase.from('tenant_entitlements').upsert(
      {
