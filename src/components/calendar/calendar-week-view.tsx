        ...
        <div className="flex-1 flex">
          {days.map((day, dayIndex) => (
            <div key={dayIndex} className="flex-1 border-r last:border-r-0">
              ...
              >
                <span className="text-xs font-medium text-gray-600">{format(day, 'EEE')}</span>
                <span
                  className={cn(
              ...
              <div className="relative">
                {hours.map((hour, hourIndex) => (
                  <div key={hourIndex} className="h-24 border-b" />
                ))}
                ...
                <div className="absolute inset-0 pointer-events-none px-1">
                  {activities
                    .filter((activity) => {
                      return (
                        format(activity.start_time, 'yyyy-MM-dd') === format(day, 'yyyy-MM-dd')
                      );
                    })
                    .map((activity) => {
                      ...
                      return (
                        <div
                          ...
                          onClick={() => onActivityClick(activity.id, activity.type)}
                          onKeyDown={(e) =>
                            e.key === 'Enter' && onActivityClick(activity.id, activity.type)
                          }
                          role="button"
                          tabIndex={0}
                          aria-label={`View ${activity.type} activity`}
                        >
                          ...
                          {activity.contact_name && (
                            <div className="opacity-90 truncate">{activity.contact_name}</div>
                          )}
                        </div>
                      );
                    })}
