...
export function CalendarMonthView({
  ...
  const getActivitiesForDay = (day: Date) => {
    return activities.filter((activity) => isSameDay(activity.start_time, day));
  };
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <span
                  className={cn('text-sm font-medium', isTodayDate && 'text-blue-600 font-bold')}
                >
                  {format(day, 'd')}
                </span>
              ...
              <div className="space-y-1">
                {dayActivities.slice(0, 3).map((activity) => {
                  return (
                    ...
                    >
                      <span className="font-medium">{format(activity.start_time, 'h:mm a')}</span>{' '}
                      {activity.title}
                    </div>
                  );
                })}
