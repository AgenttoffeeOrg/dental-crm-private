...
export function EnterpriseDealsTable({
  ...
  const loadDeals = async () => {
    ...
    try {
      ...
      if (ownerFilter !== 'all') {
        if (ownerFilter === 'my') {
          ...
          query = query
            ...
            .neq('owner_user_id', currentUserId || appUser?.id);
        } else {
      ...
      console.info(
        `[EnterpriseDealsTable] Loaded ${data?.length || 0} deals for tenant ${tenantId}`
      );
  ...
  const handleBulkDelete = async () => {
    ...
    const confirmed = confirm(
      `Are you sure you want to delete ${selectedDealIds.size} deal(s)? This action cannot be undone.`
    );
    if (!confirmed) return;
  ...
  const getAgingBadge = (days: number, status: string) => {
    ...
    return (
      <Badge className={cn('text-xs font-medium border shadow-sm', colorClass)}>{days}d</Badge>
    );
  };
            ...
            <div className="flex items-center justify-between">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {subtitle ||
                    `Manage ${LABELS.DEAL.verbPlural} across all pipelines • ${format.number(totalCount)} total`}
                </p>
              </div>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="outline"
                      size="sm"
                      className="h-9 w-[95px] justify-between border-gray-300 rounded-md text-sm shadow-sm"
                    >
              ...
              {loading ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-gray-500">
                  ...
                  <p className="text-sm mt-2">
                    Create a pipeline with stages to view deals in board view
                  </p>
                </div>
              ) : (
                stages
                  ...
                  .map((stage) => {
                    ...
                    const stageValue = stageDeals.reduce(
                      (sum, d) => sum + (d.value_estimate_cents || 0),
                      0
                    );
                    ...
                    return (
                      ...
                      >
                        <div className="p-4 border-b border-gray-200 bg-white rounded-t-lg">
                          ...
                          <p className="text-sm text-gray-600">{formatCurrencyValue(stageValue)}</p>
                        </div>
                          ...
                          >
                            <div className="flex flex-col items-center gap-3">
                              ...
                              <p className="text-base font-medium text-gray-700">
                                No {LABELS.DEAL.verbPlural} found
                              </p>
                          ...
                          >
                            ...
                            {isColumnVisible('pipeline') && mode === 'universal' && (
                              <td className="px-6 py-4">
                                <Badge
                                  variant="outline"
                                  className="text-xs font-medium border-gray-200 text-gray-700 bg-gray-50/50 px-2.5 py-1"
                                >
                                  ...
                                  <div className="flex flex-wrap gap-1.5">
                                    {deal.treatment_tags.slice(0, 2).map((tag) => (
                                      <Badge
                                        key={tag}
                                        variant="outline"
                                        className="text-xs font-normal border-gray-200 bg-white px-2 py-0.5"
                                      >
                                        {tag}
                                      </Badge>
                                    ))}
                              ...
                              >
                                ...
                                {deal.pipeline_id && (
                                  <SimpleDropdownMenuItem
                                    onClick={() => {
                                      router.push(
                                        `/pipeline?pipeline=${deal.pipeline_id}&highlight=${deal.id}`
                                      );
                                    }}
                                  >
...
function DraggableDealCard({
  ...
  return (
    ...
    >
      ...
      {deal.contact && <p className="text-xs text-gray-600 mb-2">{deal.contact.full_name}</p>}
