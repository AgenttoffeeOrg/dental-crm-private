...
export function ConversionIntelligenceDashboard({ tenantId }: { tenantId?: string }) {
  ...
  const loadData = async () => {
    ...
    setLoading(true)
    try {
      await Promise.all([loadSummary(tenantId), loadScriptMetrics(tenantId), loadPersonaSignals(tenantId), loadStageMetrics(tenantId)])
  ...
  const loadSummary = async (tenantId: string) => {
    ...
    data.forEach((deal: any) => {
      ...
      const isWon = deal.status === 'won' || stageName.includes('won') || stageName.includes('closed')
      if (isWon) {
        dealsWon += 1
  ...
  const loadScriptMetrics = async (tenantId: string) => {
    ...
    setScripts(Array.from(aggregate.values()).sort((a, b) => {
  ...
  const loadPersonaSignals = async (tenantId: string) => {
    ...
    data.forEach((row: any) => {
      const tags: string[] = Array.isArray(row.snapshot?.persona_tags) ? row.snapshot.persona_tags : []
      tags.forEach((tag) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Sales Performance & Conversion Intelligence</h2>
          ...
          <CardContent>
            {scripts.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={320}>
                <BarChart data={scripts.slice(0, 6).map((script) => ({
                  name: script.title?.slice(0, 28) ?? 'Script',
                  successRate: script.usages > 0 ? (script.successful_outcomes / script.usages) * 100 : 0,
        ...
        <Card>
          ...
          <CardContent>
            {stageMetrics.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={320}>
                <LineChart data={stageMetrics.slice(0, 10).map((stage) => ({
                  ...
                  <Tooltip />
                  <Legend />
                  <Line yAxisId="left" type="monotone" dataKey="value" stroke="#818cf8" name="Value (£)" />
                  <Line yAxisId="right" type="monotone" dataKey="velocity" stroke="#f97316" name="Days in Stage" />
                </LineChart>
              </ResponsiveContainer>
            )}
          </CardContent>
            ...
            {bestScript ? (
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Success rate {bestScript.successRate.toFixed(1)}% · {bestScript.wins} wins recorded
                  </p>
                </div>
                <p className="text-sm text-gray-700">
                  Continue reinforcing this playbook with similar personas and monitor revenue impact via the script metrics panel.
                </p>
              </div>
            ) : (
