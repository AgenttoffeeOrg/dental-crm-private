...
function buildProviderGroup(provider: string, connections: any[]): ProviderGroup {
  ...
  const groupConnections =
    connections?.filter((c) => providerIntegrations.some((i) => i.type === c.integration_type)) ||
    [];
...
export function UnifiedIntegrationsHub() {
  ...
  const filteredGroups = providerGroups.filter((group) => {
    if (selectedCategory !== 'all') {
      const categoryIntegrations = getIntegrationsByCategory(
        selectedCategory as UnifiedIntegration['category']
      );
  ...
  const totalConnected = providerGroups.reduce(
    (sum, group) => sum + group.integrations.filter((i) => i.configured).length,
    0
  );
    ...
    <div className="space-y-6">
      ...
      <Alert className="border-blue-200 bg-blue-50">
        ...
        <AlertDescription className="text-blue-800">
          Connect all your tools in one place. One connection per provider = All services from that
                        ...
                        {INTEGRATION_GROUPS[group.provider] ? (
                          <Button
                            ...
                            onClick={() => {
                              ...
                              const oauthIntegration = group.integrations.find(
          ...
          {selectedIntegration && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>{selectedIntegration.simpleDescription}</DialogDescription>
              </DialogHeader>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          toast.info('Opened signup page', {
                            description:
                              'After you create your account, come back and click "I Have My Account"',
                          });
                        }}
                      >
                        Open Signup Page
              ...
              {(wizardStep === 2 || (!selectedIntegration.buyUrl && wizardStep === 1)) &&
                selectedIntegration.apiKeyFields && (
                  <div className="space-y-4">
                    <Alert>
                      <AlertTitle>
                        Step {selectedIntegration.buyUrl ? 2 : 1}: Enter Your Credentials
                      </AlertTitle>
                      <AlertDescription>
                          ...
                          {field.type === 'password' ? (
                            <div className="flex gap-2">
                              <Input
                                ...
                                onChange={(e) =>
                                  setApiKeyData({ ...apiKeyData, [field.key]: e.target.value })
                                }
                              />
                              <Button
                                variant="outline"
                    ...
                    {selectedIntegration.webhookUrl && (
                      <Alert className="border-purple-200 bg-purple-50">
                        ...
                        <AlertDescription className="text-purple-800">
                          <p className="mb-2">
                            Set this webhook URL in your {selectedIntegration.name} dashboard:
                          </p>
                    ...
                    {connectionTestResult === 'error' && (
                      <Alert className="border-red-200 bg-red-50">
                        ...
                        <AlertDescription className="text-red-800">
                          Please check your credentials. Make sure you copied them correctly from
