 ...
 *
 * Phase 2b.7 — settings/email auth gate tests. Mocks the auth helper
 * boundary and the Supabase service client so this stays a unit test
 * (no real Supabase, no real SMTP). 5 cases — settings routes are
 * NOT rate-limited per locked decision §0.3, so the 429 case from
 * 2b.5 is intentionally absent here.
 */
...
// Supabase builder mock: track .from('tenants').update(...).eq('id', ...)
// .select().single(). Each call returns the chained shape; jest spies
// capture argument values for the assertions below.
...
function makeReq(body: Record<string, unknown>): NextRequest {
  return new NextRequest('http://localhost/api/settings/email', {
    method: 'PATCH',
...
describe('PATCH /api/settings/email — auth gate', () => {
  ...
  it('returns 200 and pins WHERE id to authenticated tenant when body tenant_id matches', async () => {
    ...
    // The WHERE filter is pinned to the AUTHENTICATED tenant id, not the body.
    expect(mockEq).toHaveBeenCalledWith('id', TENANT)
    // The update payload must NOT carry tenant_id (it was destructured out).
    const updatePayload = mockUpdate.mock.calls[0][0]
    expect(updatePayload).not.toHaveProperty('tenant_id')
    expect(updatePayload.smtp_host).toBe('smtp.example.com')
  })
