 ...
 *
 * Phase 2b.7 — settings/whatsapp auth gate tests. Mocks the auth
 * helper boundary and the Supabase service client so this stays a
 * unit test. 5 cases — settings routes are NOT rate-limited per
 * locked decision §0.3.
 */
...
function makeReq(body: Record<string, unknown>): NextRequest {
  return new NextRequest('http://localhost/api/settings/whatsapp', {
    method: 'PATCH',
...
const validBody = {
  ...
  whatsapp_api_key: 'wa_key_test',
  whatsapp_api_secret: 'wa_secret_test',
}
...
describe('PATCH /api/settings/whatsapp — auth gate', () => {
  ...
  it('returns 403 (entitlement_missing) when helper rejects on entitlement', async () => {
    mockRequire.mockRejectedValueOnce(
      new FakeAuthApiError(403, 'entitlement_missing', 'Tenant does not have whatsapp enabled')
    )
    const res = await PATCH(makeReq({ ...validBody, tenant_id: TENANT }))
  ...
  it('returns 200 and pins WHERE id to authenticated tenant when body tenant_id matches', async () => {
    ...
    expect(updatePayload.whatsapp_phone_number).toBe('+15551234567')
  })
