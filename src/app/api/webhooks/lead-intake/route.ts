...
async function categorizeLead(message: string, tenantId: string, supabase: any) {
  try {
    const { data, error } = await supabase.rpc('auto_categorize_lead', {
      p_message: message,
    ...
    if (error) {
      console.warn(
        '[lead-intake] categorization RPC returned error (best-effort, lead capture unaffected)',
        {
...
function calculateLeadScore(data: any): number {
  ...
  if (
    data.message &&
    emergencyKeywords.some((keyword) => data.message.toLowerCase().includes(keyword))
  ) {
    score += 30;
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (sourceError) {
      ...
      return NextResponse.json({ error: 'Lead source not configured' }, { status: 400 });
    }
    ...
    const { data: existingContact } = await supabase
      ...
      .eq('tenant_id', tenantId)
      .or(
        `primary_email.eq.${validatedData.contact.email},primary_phone.eq.${validatedData.contact.phone}`
      )
      .single();
    ...
    if (intakeError) {
      ...
      return NextResponse.json({ error: 'Failed to create lead intake' }, { status: 500 });
    }
    ...
    if (leadScore >= 80) {
      try {
        ...
        if (validatedData.treatment_tags && validatedData.treatment_tags.length > 0) {
          ...
          try {
            const extractionResult = await extractTagsFromDealText(validatedData.message, tenantId);
            ...
            console.warn(
              '[lead-intake] AI tag extraction failed (best-effort, will route unsorted)',
              {
                route: '/api/webhooks/lead-intake',
                tenant_id: tenantId,
        ...
        console.log(
          `[Lead Intake] Routed to pipeline: ${routingResult.pipelineId} (${routingResult.routingMethod})`
        );
        ...
        if (dealError) {
          ...
          console.error(
            '[lead-intake] deal creation failed (lead already captured; deal recoverable)',
            {
              route: '/api/webhooks/lead-intake',
              tenant_id: tenantId,
              lead_intake_id: leadIntake.id,
          ...
          console.log(
            `[Lead Intake] Created deal ${newDeal?.id} with ${treatmentTags.length} tags`
          );
        }
      } catch (routingError) {
        // Same rationale as above: lead/contact persisted; deal routing is best-effort.
        console.error(
          '[lead-intake] deal routing threw (lead already captured; deal recoverable)',
          {
            route: '/api/webhooks/lead-intake',
            tenant_id: tenantId,
            lead_intake_id: leadIntake.id,
