  ...
  try {
    ...
    if (formData.treatment_tags && Array.isArray(formData.treatment_tags)) {
      ...
      if (formText.trim()) {
        try {
          ...
          console.warn(
            '[form-submission] AI tag extraction failed (best-effort, will route unsorted)',
            {
              route: '/api/webhooks/form-submission',
    ...
    if (contactId) {
      try {
        ...
        console.log(
          `[Form Webhook] Routed to pipeline: ${routingResult.pipelineId} (${routingResult.routingMethod})`
        );
        ...
        throw new Error(
          `Deal routing failed: ${routingError instanceof Error ? routingError.message : String(routingError)}`
        );
      }
    }
