 ...
 *
 * Phase 2b.5 — send-whatsapp-v2 auth gate tests.
 */
...
jest.mock('@/lib/whatsapp-service', () => ({
  whatsappService: {
    ...
    send: (...args: unknown[]) => mockWaSend(...args),
  },
}))
...
function makeReq(body: Record<string, unknown>): NextRequest {
  return new NextRequest('http://localhost/api/communications/send-whatsapp-v2', {
    method: 'POST',
...
beforeEach(() => {
  ...
  mockTenantSingle.mockResolvedValue({
    data: {
      whatsapp_api_key: 'k',
      whatsapp_api_secret: 's',
      whatsapp_phone_number: 'whatsapp:+14155238886',
    },
    error: null,
  })
  mockWaSend.mockResolvedValue({ messageId: 'wa-1' })
})

describe('POST /api/communications/send-whatsapp-v2 — auth gate', () => {
  ...
  it('returns 200 and dispatches via whatsappService', async () => {
    const res = await POST(makeReq({ ...validBody, tenant_id: TENANT }))
