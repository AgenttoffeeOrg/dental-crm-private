...
} from '@/lib/queues/deadletter-service';
...
export async function GET(request: NextRequest) {
  if (!queueManager.isEnabled()) {
    return NextResponse.json({ enabled: false, message: 'Queues disabled' }, { status: 200 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to list dead-letter jobs' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  if (!action || !jobId || !queueParam) {
    return NextResponse.json({ error: 'action, queue and jobId are required' }, { status: 400 });
  }

  const deadLetterQueue = resolveDeadLetterQueue(queueParam);

  if (!deadLetterQueue) {
    return NextResponse.json({ error: 'Unknown dead-letter queue' }, { status: 400 });
  }
