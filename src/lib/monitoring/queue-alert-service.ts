...
import type { QueueAlertRule, QueueHealthIncident } from '@/types/database';
...
export async function evaluateQueueRule(
  ...
  const severity: 'warning' | 'critical' = breaches.some(
    (breach) => breach.metric === 'failed' || breach.metric === 'oldest_waiting'
  )
    ? 'critical'
    : 'warning';
  ...
  if (existingIncident) {
    ...
    if (!error && data) {
      ...
      console.error('[queue-alerts] Failed to update incident', {
        error,
