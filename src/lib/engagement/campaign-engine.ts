...
function nextStep(
  current: EngagementStep | undefined,
...
export class CampaignEngine {
  ...
  async processEnrollment(enrollmentId: string): Promise<void> {
    ...
    if (!stepToExecute) {
      ...
      await this.logEvent(bundle, null, 'campaign.completed', 'completed', {
        reason: 'no_steps_remaining',
      });
      return;
    }
  ...
  private async loadEnrollmentBundle(enrollmentId: string): Promise<EnrollmentBundle | null> {
    ...
    const [{ data: campaign }, { data: steps }, { data: contact }, { data: deal }] =
      await Promise.all([
  ...
  private async executeStep(
    ...
    const updatePayload: Partial<EngagementEnrollment> & {
      updated_at: string;
      last_run_at: string;
    } = {
      updated_at: now,
    ...
    if (followUpStep && updatePayload.status !== 'completed') {
      ...
      await this.logEvent(bundle, step, 'campaign.completed', 'completed', {
  ...
  private async runEmailStep(bundle: EnrollmentBundle, step: EngagementStep): Promise<void> {
    if (!bundle.contact?.primary_email) {
      await this.logEvent(bundle, step, 'step.email_skipped', 'failed', {
    ...
    const html =
      step.config?.html || step.config?.content || '<p>Hello from Dental Sales Coach</p>';
  ...
  private async runSmsStep(
    ...
    if (!phone) {
      await this.logEvent(bundle, step, 'step.message_skipped', 'failed', {
    ...
    const message =
      step.config?.message || step.config?.content || 'Hello from Dental Sales Coach.';
  ...
  private async runAiMessageStep(bundle: EnrollmentBundle, step: EngagementStep): Promise<void> {
    if (!isOpenAIConfigured()) {
      await this.logEvent(bundle, step, 'step.ai_skipped', 'failed', {
    ...
    if (channel === 'email') {
      ...
      await this.runSmsStep(
        ...
        {
          ...step,
          step_type: 'send_whatsapp' as EngagementStepType,
          config: { ...(step.config || {}), message: aiMessage },
        },
        true
      );
    } else {
      await this.runSmsStep(
        ...
        {
          ...step,
          step_type: 'send_sms' as EngagementStepType,
          config: { ...(step.config || {}), message: aiMessage },
        },
        false
      );
  ...
  private async runWebhookStep(bundle: EnrollmentBundle, step: EngagementStep): Promise<void> {
    ...
    if (!endpoint) {
      await this.logEvent(bundle, step, 'step.webhook_skipped', 'failed', {
        reason: 'missing_url',
      });
  ...
  private evaluateBranch(
    ...
    if (Array.isArray(branchConfig?.rules)) {
      ...
      const matches = this.evaluateCondition(
        value,
        branchConfig.condition.operator,
        branchConfig.condition.value
      );
