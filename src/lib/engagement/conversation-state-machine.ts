...
function detectEscalationIntent(turn: ConversationTurn): boolean {
  return (
    containsKeyword(turn.message, HUMAN_ESCALATION_KEYWORDS) ||
    (turn.intent === 'human_escalation' && (turn.confidence || 0) > 0.6)
  );
}

function detectBookingIntent(turn: ConversationTurn): boolean {
  return (
    containsKeyword(turn.message, BOOKING_KEYWORDS) ||
    (turn.intent === 'appointment_request' && (turn.confidence || 0) > 0.5)
  );
}
...
export function evaluateConversationTurn(
  ...
  const turns = [...context.lastTurns].sort(
  ...
  if (latest.role === 'patient') {
    if (detectEscalationIntent(latest)) {
      ...
      actions.push({
        type: 'assign_human',
        reason: 'Negative sentiment identified, safer to escalate',
      });
      nextState = 'handoff_pending';
  ...
  if (
    context.activeCampaignIds &&
    context.activeCampaignIds.length === 0 &&
  ...
  const shouldClose =
    ...
    (latest.role === 'bot' &&
      containsKeyword(latest.message, ['thank you', 'that helps', 'all good']));

  if (shouldClose) {
    actions.push({
      type: 'close_session',
      reason: 'Conversation resolved or human agent took over',
      status: 'closed',
    });
    nextState = 'closed';
  ...
  if (actions.length === 0) {
    ...
    actions.push({
      type: 'ai_reply',
      prompt:
        'Provide a concise, helpful response acknowledging the patient and offering next steps.',
      tone: 'empathetic',
      channel: context.channel,
    });
  }
