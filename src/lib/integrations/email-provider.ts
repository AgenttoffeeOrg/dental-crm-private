...
async function refreshGoogleAccessToken(refreshToken: string) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Google token refresh failed: ${error.error_description || error.error || response.statusText}`
    );
  }
...
async function refreshMicrosoftAccessToken(refreshToken: string) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new Error(
      `Microsoft token refresh failed: ${error.error_description || error.error || response.statusText}`
    );
  }
...
async function sendViaGmail(
  ...
  if (settings.email_oauth_token && !isTokenExpired(settings.email_oauth_expires_at)) {
    oauth2Client.setCredentials({
      ...
      expiry_date: settings.email_oauth_expires_at
        ? new Date(settings.email_oauth_expires_at).getTime()
        : undefined,
    });
  } else {
...
async function sendViaOutlook(
  ...
  const message = {
    ...
    bccRecipients:
      payload.bcc && payload.bcc.length > 0 ? payload.bcc.map(createRecipient) : undefined,
