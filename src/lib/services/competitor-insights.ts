...
export async function getCompetitorInsights(
  ...
  const priceAccumulator = new Map<
    string,
    { latest: CompetitorPricePoint; previous?: CompetitorPricePoint }
  >();
  ...
  priceAccumulator.forEach(({ latest, previous }) => {
    ...
    if (
      ...
      compareTimestamps(latest.collected_at, competitor.lastPriceUpdate) > 0
    ) {
  ...
  for (const touchpoint of touchpoints) {
    ...
    if (
      !competitor.lastTouchpoint ||
      compareTimestamps(touchpoint.occurred_at, competitor.lastTouchpoint) > 0
    ) {
      competitor.lastTouchpoint = touchpoint.occurred_at ?? competitor.lastTouchpoint;
    }

    touchpointTotals.set(
      touchpoint.competitor_id,
      (touchpointTotals.get(touchpoint.competitor_id) ?? 0) + 1
    );
  ...
  const competitorsArray = Array.from(competitorMap.values())
    .map((competitor) => ({
      ...competitor,
      latestPrices: competitor.latestPrices.sort((a, b) =>
        compareTimestamps(b.collectedAt, a.collectedAt)
      ),
    }))
  ...
  const touchpointsByType: CompetitorTouchpointSummary[] = Array.from(
    touchpointTypeTotals.entries()
  )
  ...
  const recentJobs: IngestionJobSummary[] = jobs.map((job) => {
    const rows =
      typeof job.payload?.rows === 'number'
        ? job.payload.rows
        : parseInt(job.payload?.rows as any, 10) || null;
    const failures =
      typeof job.result_summary?.failures === 'number'
...
function determineTopCompetitor(
  ...
  competitors.forEach((competitor) => {
    ...
    const priceScore = competitor.latestPrices.reduce(
      (acc, price) => acc + Math.abs(price.deltaCents ?? 0),
      0
    );
